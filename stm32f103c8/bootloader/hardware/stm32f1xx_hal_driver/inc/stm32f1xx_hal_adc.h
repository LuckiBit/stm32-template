...
#include "stm32f1xx_hal_def.h"
     ...
     * state. ADC can be either disabled or enabled without conversion on going on regular group.
     */
    typedef struct
    {
        ...
        uint32_t
            ScanConvMode; /*!< Configures the sequencer of regular and injected groups.
                             ...
                             can be a value of @ref ADC_Scan_mode Note: For regular group, this
                             parameter should be enabled in conversion either by polling
                             (HAL_ADC_Start with Discontinuous mode and NbrOfDiscConversion=1) or by
                             DMA (HAL_ADC_Start_DMA), but not by interruption (HAL_ADC_Start_IT): in
                             scan mode, interruption is triggered only on the the last conversion of
                             the sequence. All previous conversions would be overwritten by the last
                             one. Injected group used with scan mode has not this constraint: each
                             rank has its own result register, no data is overwritten. */
        FunctionalState
        ...
        uint32_t ExternalTrigConv; /*!< Selects the external event used to trigger the conversion
                                      ...
                                      triggering is on event rising edge. This parameter can be a
    ...
    typedef struct
    {
        uint32_t Channel; /*!< Specifies the channel to configure into ADC regular group.
                             ...
                             internal measurement channels (VrefInt/TempSensor) Note: On STM32F10xx8
                             and STM32F10xxB devices: A low-amplitude voltage glitch may be
                             generated (on ADC input 0) on the PA0 pin, when the ADC is converting
                             with injection trigger. It is advised to distribute the analog channels
                             so that Channel 0 is configured as an injected channel. Refer to errata
                             sheet of these devices for more details. */
        uint32_t
            Rank; /*!< Specifies the rank in the regular group sequencer
        ...
        uint32_t
            SamplingTime; /*!< Sampling time value to be set for the selected channel.
                             ...
                             (12.5 ADC clock cycles at ADC resolution 12 bits). This parameter can
                             be a value of @ref ADC_sampling_times Caution: This parameter updates
                             the parameter property of the channel, that can be used into regular
                             and/or injected groups. If this same channel has been previously
                             configured in the other group (regular/injected), it will be updated to
                             last setting. Note: In case of usage of internal measurement channels
                             (VrefInt/TempSensor), sampling time constraints must be respected
                             ...
                             parameters TS_vrefint, TS_temp (values rough order: 5us to 17.1us min).
                           */
     ...
     *         ADC state can be either disabled or enabled without conversion on going on regular
     * and injected groups.
     */
    typedef struct
    {
        uint32_t WatchdogMode; /*!< Configures the ADC analog watchdog mode: single/all channels,
                                  regular/injected group. This parameter can be a value of @ref
                                  ADC_analog_watchdog_mode. */
        uint32_t Channel;      /*!< Selects which ADC channel to monitor by analog watchdog.
                                    This parameter has an effect only if watchdog mode is configured on
                                  single channel (parameter WatchdogMode)      This parameter can be a value
                                  of @ref ADC_channels. */
        FunctionalState
            ITMode; /*!< Specifies whether the analog watchdog is configured in interrupt or polling
...
#define HAL_ADC_STATE_REG_BUSY                                                                     \
    0x00000100U /*!< A conversion on group regular is ongoing or can occur (either by continuous   \
                   mode, external trigger, low power auto power-on, multimode ADC master control)                          \
                 */
#define HAL_ADC_STATE_REG_EOC 0x00000200U /*!< Conversion data available on group regular */
#define HAL_ADC_STATE_REG_OVR                                                                      \
    0x00000400U /*!< Not available on STM32F1 device: Overrun occurrence */
#define HAL_ADC_STATE_REG_EOSMP                                                                    \
    0x00000800U /*!< Not available on STM32F1 device: End Of Sampling flag raised  */

/* States of ADC group injected */
#define HAL_ADC_STATE_INJ_BUSY                                                                     \
    0x00001000U /*!< A conversion on group injected is ongoing or can occur (either by             \
                   auto-injection mode, external trigger, low power auto power-on, multimode ADC                                          \
                   master control) */
...
#define HAL_ADC_STATE_AWD2                                                                         \
    0x00020000U /*!< Not available on STM32F1 device: Out-of-window occurrence of analog watchdog  \
                   2 */
#define HAL_ADC_STATE_AWD3                                                                         \
    0x00040000U /*!< Not available on STM32F1 device: Out-of-window occurrence of analog watchdog  \
                   3 */
...
#define HAL_ADC_STATE_MULTIMODE_SLAVE                                                              \
    ...
    typedef struct __ADC_HandleTypeDef
    {
        ...
        void (*InjectedConvCpltCallback)(struct __ADC_HandleTypeDef *hadc);
            /*!< ADC group injected conversion complete callback */  /*!< ADC end of sampling
                                                                        callback */
...
#define ADC_CHANNEL_15                                                                             \
    ((uint32_t)(ADC_SQR3_SQ1_3 | ADC_SQR3_SQ1_2 | ADC_SQR3_SQ1_1 | ADC_SQR3_SQ1_0))
...
#define ADC_CHANNEL_TEMPSENSOR                                                                     \
    ADC_CHANNEL_16                         /* ADC internal channel (no connection on device pin) */
...
#define ADC_SAMPLETIME_239CYCLES_5                                                                 \
    ((uint32_t)(ADC_SMPR2_SMP0_2 | ADC_SMPR2_SMP0_1 |                                              \
...
#define ADC_AWD1_EVENT                                                                             \
    ADC_AWD_EVENT /*!< ADC Analog watchdog 1 event: Alternate naming for compatibility with other  \
                     STM32 devices having several analog watchdogs */
/**
 * @}
 */
...
#define ADC_CR1_SCAN_SET(_SCAN_MODE_)                                                              \
    ((((_SCAN_MODE_) == ADC_SCAN_ENABLE) || ((_SCAN_MODE_) == ENABLE)) ? (ADC_SCAN_ENABLE)         \
                                                                       : (ADC_SCAN_DISABLE))
...
#define ADC_CONVCYCLES_MAX_RANGE(__HANDLE__)                                                       \
    (((((__HANDLE__)->Instance->SMPR2 & ADC_SAMPLETIME_ALLCHANNELS_SMPR2BIT2) == RESET) &&         \
      (((__HANDLE__)->Instance->SMPR1 & ADC_SAMPLETIME_ALLCHANNELS_SMPR1BIT2) == RESET))           \
         ...
         : ((((((__HANDLE__)->Instance->SMPR2 & ADC_SAMPLETIME_ALLCHANNELS_SMPR2BIT1) == RESET) && \
              (((__HANDLE__)->Instance->SMPR1 & ADC_SAMPLETIME_ALLCHANNELS_SMPR1BIT1) ==           \
               RESET)) ||                                                                          \
...
#define IS_ADC_REGULAR_RANK(CHANNEL)                                                               \
    (((CHANNEL) == ADC_REGULAR_RANK_1) || ((CHANNEL) == ADC_REGULAR_RANK_2) ||                     \
     ...
     ((CHANNEL) == ADC_REGULAR_RANK_9) || ((CHANNEL) == ADC_REGULAR_RANK_10) ||                    \
     ((CHANNEL) == ADC_REGULAR_RANK_11) || ((CHANNEL) == ADC_REGULAR_RANK_12) ||                   \
     ((CHANNEL) == ADC_REGULAR_RANK_13) || ((CHANNEL) == ADC_REGULAR_RANK_14) ||                   \
...
/** @defgroup ADC_regular_discontinuous_mode_number_verification ADC regular discontinuous mode
 * number verification
 * @{
 */
