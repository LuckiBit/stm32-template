    ...
    in asynchronous IrDA mode.
      (+) For the asynchronous mode only these parameters can be configured:
        ...
        (++) Prescaler: A pulse of width less than two and greater than one PSC period(s) may or may
             not be rejected. The receiver set up time should be managed by software. The IrDA
physical layer specification specifies a minimum of 10 ms delay between transmission and reception
(IrDA is a half duplex protocol).
  ...
  ==============================================================================
    ...
    (#) There are two modes of transfer:
       ...
       (++) Non-Blocking mode: The communication is performed using Interrupts
           ...
           The HAL_IRDA_ErrorCallback() user callback will be executed when a communication error is
detected
...
executed. Transfer is kept ongoing on IRDA side. If user wants to abort it, Abort services should be
called by user.
 ...
 */
HAL_StatusTypeDef
HAL_IRDA_Transmit(IRDA_HandleTypeDef *hirda, const uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
    const uint16_t *tmp;
 ...
 */
HAL_StatusTypeDef
HAL_IRDA_Transmit_IT(IRDA_HandleTypeDef *hirda, const uint8_t *pData, uint16_t Size)
{
    /* Check that a Tx process is not already ongoing */
    if(hirda->gState == HAL_IRDA_STATE_READY)
    {
 ...
 */
HAL_StatusTypeDef
HAL_IRDA_Transmit_DMA(IRDA_HandleTypeDef *hirda, const uint8_t *pData, uint16_t Size)
{
    const uint32_t *tmp;
    ...
    if(hirda->gState == HAL_IRDA_STATE_READY)
    {
        ...
        if(HAL_DMA_Start_IT(hirda->hdmatx,
                            *(const uint32_t *)tmp,
                            (uint32_t)&hirda->Instance->DR,
                            Size) == HAL_OK)
        {
...
HAL_StatusTypeDef HAL_IRDA_Receive_DMA(IRDA_HandleTypeDef *hirda, uint8_t *pData, uint16_t Size)
{
    ...
    if(hirda->RxState == HAL_IRDA_STATE_READY)
    {
        ...
        tmp = (uint32_t *)&pData;
        if(HAL_DMA_Start_IT(hirda->hdmarx,
                            (uint32_t)&hirda->Instance->DR,
                            *(uint32_t *)tmp,
                            Size) == HAL_OK)
        {
            /* Clear the Overrun flag just before enabling the DMA Rx request: can be mandatory for
             * the second transfer */
...
HAL_StatusTypeDef HAL_IRDA_DMAStop(IRDA_HandleTypeDef *hirda)
{
    ...
    /* The Lock is not implemented on this API to allow the user application
       ...
       HAL_IRDA_RxCpltCallback(): when calling HAL_DMA_Abort() API the DMA TX/RX Transfer complete
       interrupt is generated and the correspond call back is executed HAL_IRDA_TxCpltCallback() /
       HAL_IRDA_RxCpltCallback()
    */
...
/**
 * @brief  End ongoing Rx transfer on IRDA peripheral (following error detection or Reception
 * completion).
 * @param  hirda IRDA handle.
 * @retval None
 */
