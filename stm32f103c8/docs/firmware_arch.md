...
| 项目 | 当前实现 |
| --- | --- |
| 目标芯片 | STM32F103C8T6，Cortex-M3，64 KiB Flash、20 KiB SRAM |
| 编译工具链 | GCC `arm-none-eabi-gcc`，C11，CMake、Ninja |
| 编译告警 | 两个工程均启用 `-Wall -Wextra` |
| 外设驱动 | 两个工程均编译 STM32F1 HAL，芯片宏为 `STM32F103xB` |
...
| Application | `0x08008000` | `0x0800FC00` | 31 KiB |
| Boot Info | `0x0800FC00` | `0x08010000` | 1 KiB，独占一个擦除页 |

SRAM 范围为 `0x20000000`～`0x20005000`，容量 20 KiB。

配置位置：

- [Bootloader 分区常量](../bootloader/core/flash_layout.h)。
- [Application 分区常量](../application/core/flash_layout.h)。
- [Bootloader 链接布局](../bootloader/core/linker/stm32f103c8.ld)。
- [Application 链接布局](../application/core/linker/stm32f103c8.ld)。

两个工程的 `core/linker/` 均只保存 C8 对应的链接脚本，CMake 直接引用该脚本。
...
FreeRTOS 移植层以及两套链接脚本和分区常量。当前只配置了 STM32F103C8T6，
