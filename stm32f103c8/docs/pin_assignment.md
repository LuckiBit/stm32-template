...
本文记录 LQFP48 封装在当前固件中的 GPIO 引脚分配。表格按 PA0、PA1……的顺序排列，仅列出该封装实际引出的 GPIO。
...
以下映射存在于通用 UART BSP 配置中，但 STM32F103C8 的 LQFP48 封装没有引出这些 GPIO，不能直接启用：

| 引脚 | 功能 | 作用 | 使用范围 |
| --- | --- | --- | --- |
| PC10 | UART4_TX | 串口 4 发送候选映射 | 当前封装不可用 |
| PC11 | UART4_RX | 串口 4 接收候选映射 | 当前封装不可用 |
| PC12 | UART5_TX | 串口 5 发送候选映射 | 当前封装不可用 |
| PD2 | UART5_RX | 串口 5 接收候选映射 | 当前封装不可用 |
