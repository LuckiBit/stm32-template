...
#define LL_RTC_TAMPER_ACTIVELEVEL_LOW                                                              \
    BKP_CR_TPAL /*!< A high level on the TAMPER pin resets all data backup registers (if TPE bit   \
                   is set) */
#define LL_RTC_TAMPER_ACTIVELEVEL_HIGH                                                             \
    (0x00000000U) /*!< A low level on the TAMPER pin resets all data backup registers (if TPE bit  \
                     is set) */
...
#define LL_RTC_CALIB_OUTPUT_RTCCLOCK                                                               \
    BKP_RTCCR_CCO /*!< Calibration output is RTC Clock with a frequency divided by 64 on the       \
                     TAMPER Pin */
#define LL_RTC_CALIB_OUTPUT_ALARM                                                                  \
    BKP_RTCCR_ASOE /*!< Calibration output is Alarm pulse signal on the TAMPER pin */
#define LL_RTC_CALIB_OUTPUT_SECOND                                                                 \
    (BKP_RTCCR_ASOS |                                                                              \
     BKP_RTCCR_ASOE) /*!< Calibration output is Second pulse signal on the TAMPER pin*/
/**
 * @}
 */
...
#define __LL_RTC_CONVERT_BCD2BIN(__VALUE__)                                                        \
    ...
    LL_RTC_BKP_SetRegister(BKP_TypeDef *BKPx, uint32_t BackupRegister, uint32_t Data)
    {
        uint32_t tmp = 0U;
     ...
     * @note   This Calibration value should be between 0 and 121 when using positive sign with a
     * 4-ppm step.
     * @retval None
     */
    ...
    ErrorStatus
    LL_RTC_ALARM_Init(RTC_TypeDef *RTCx, uint32_t RTC_Format, LL_RTC_AlarmTypeDef *RTC_AlarmStruct);
