 ...
 * (SPI_CR1_SPE bit =0), SPI peripheral should be in disabled state prior calling this function.
...
ErrorStatus LL_I2S_Init(SPI_TypeDef *SPIx, const LL_I2S_InitTypeDef *I2S_InitStruct)
{
    ...
    if(LL_I2S_IsEnabled(SPIx) == 0x00000000U)
    {
        ...
        /* If the requested audio frequency is not the default, compute the prescaler (i2sodd,
         * i2sdiv) else, default values are used:  i2sodd = 0U, i2sdiv = 2U.
         */
...
void LL_I2S_ConfigPrescaler(SPI_TypeDef *SPIx, uint32_t PrescalerLinear, uint32_t PrescalerParity)
{
    ...
    MODIFY_REG(SPIx->I2SPR,
               SPI_I2SPR_I2SDIV | SPI_I2SPR_ODD,
               PrescalerLinear | (PrescalerParity << 8U));
}
