  ...
  ******************************************************************************
  ...
  the DMA Tx/Rx channel.
    ...
    (@) The specific SMARTCARD interrupts (Transmission complete interrupt,
        ...
        __HAL_SMARTCARD_ENABLE_IT() and __HAL_SMARTCARD_DISABLE_IT() inside the transmit and receive
  process.
...
/** @defgroup SMARTCARD_Exported_Functions_Group1 SmartCard Initialization and de-initialization
functions
...
error is detected
    ...
    (#) For Abort services based on interrupts (HAL_SMARTCARD_Abortxxx_IT), a set of Abort Complete
                                         ...
                                         uint32_t                 Timeout)
{
    ...
    if(hsc->gState == HAL_SMARTCARD_STATE_READY)
    {
        ...
        {
            hsc->TxXferCount--;
            if(SMARTCARD_WaitOnFlagUntilTimeout(hsc,
                                                SMARTCARD_FLAG_TXE,
                                                RESET,
                                                tickstart,
                                                Timeout) != HAL_OK)
        ...
        if(SMARTCARD_WaitOnFlagUntilTimeout(hsc, SMARTCARD_FLAG_TC, RESET, tickstart, Timeout) !=
           HAL_OK)
        {
            return HAL_TIMEOUT;
        }
 ...
 */
HAL_StatusTypeDef
HAL_SMARTCARD_Receive(SMARTCARD_HandleTypeDef *hsc, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
    uint8_t *tmp       = pData;
    uint32_t tickstart = 0U;
...
HAL_StatusTypeDef HAL_SMARTCARD_Abort_IT(SMARTCARD_HandleTypeDef *hsc)
{
    ...
    /* If DMA Tx and/or DMA Rx Handles are associated to SMARTCARD Handle, DMA Abort complete
