...
#define __LL_USART_DIV_SAMPLING16(__PERIPHCLK__, __BAUDRATE__)                                     \
     ...
     * the status flags, in the USARTx_SR are set to their default values.
    ...
    LL_USART_SetBaudRate(USART_TypeDef *USARTx, uint32_t PeriphClk, uint32_t BaudRate)
    {
        USARTx->BRR = (uint16_t)(__LL_USART_DIV_SAMPLING16(PeriphClk, BaudRate));
    }
     ...
     * framing error, overrun error or noise flag (FE=1 or ORE=1 or NF=1 in the USARTx_SR register).
