...
/* Exported types ------------------------------------------------------------*/
    ...
    typedef struct
    {
        ...
        uint32_t ClockSpeed; /*!< Specifies the clock frequency.
                                ...
                                @ref LL_I2C_SetClockPeriod() or @ref LL_I2C_SetDutyCycle() or @ref
        ...
        uint32_t TypeAcknowledge; /*!< Specifies the ACKnowledge or Non ACKnowledge condition after
                                     ...
                                     function @ref LL_I2C_AcknowledgeNextData(). */
...
#define __LL_I2C_10BIT_HEADER_READ(__ADDRESS__)                                                    \
     ...
     *
     * @note   Any of these events will generate interrupt if Buffer interrupts are enabled
     * too(using unitary function @ref LL_I2C_EnableIT_BUF()) : Receive buffer not empty (RXNE)
     ...
     * too(using unitary function @ref LL_I2C_EnableIT_EVT()) : Receive buffer not empty (RXNE)
     *         Transmit buffer empty (TXE)
    ...
    /**
     * @brief  Indicate the status of Address sent (master mode) or Address matched flag (slave
     * mode).
     ...
     * @note   This status is cleared by hardware after a STOP condition or repeated START
     * condition.
     ...
     * @note   In that case, the ACK bit controls the (N)ACK of the next byte received or the PEC
     * bit indicates that the next byte in shift register is a PEC.
     ...
     * @note   In that case, the ACK bit controls the (N)ACK of the current byte received or the PEC
     * bit indicates that the current byte in shift register is a PEC.
    ...
    /**
     * @brief  Enable transfer or internal comparison of the SMBus Packet Error byte (transmission
     * or reception mode).
     * @note   Macro IS_SMBUS_ALL_INSTANCE(I2Cx) can be used to check whether or not
     *         SMBus feature is supported by the I2Cx Instance.
    ...
    /**
     * @brief  Disable transfer or internal comparison of the SMBus Packet Error byte (transmission
     * or reception mode).
     * @note   Macro IS_SMBUS_ALL_INSTANCE(I2Cx) can be used to check whether or not
     *         SMBus feature is supported by the I2Cx Instance.
    ...
    /**
     * @brief  Check if the SMBus Packet Error byte transfer or internal comparison is requested or
     * not.
     * @note   Macro IS_SMBUS_ALL_INSTANCE(I2Cx) can be used to check whether or not
     *         SMBus feature is supported by the I2Cx Instance.
