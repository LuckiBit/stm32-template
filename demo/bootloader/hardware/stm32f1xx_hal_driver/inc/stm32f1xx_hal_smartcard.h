...
#include "stm32f1xx_hal_def.h"
    ...
    typedef struct
    {
        ...
        uint32_t Prescaler; /*!< Specifies the SmartCard Prescaler value used for dividing the
                               system clock to provide the smartcard clock. The value given in the
                               register (5 significant bits) is multiplied by 2 to give the division
                               factor of the source clock frequency. This parameter can be a value
                               of @ref SMARTCARD_Prescaler */

        uint32_t GuardTime; /*!< Specifies the SmartCard Guard Time value in terms of number of baud
                               clocks */
     ...
     * @note  HAL SMARTCARD State value is a combination of 2 different substates: gState and
     * RxState.
    ...
    typedef struct __SMARTCARD_HandleTypeDef
    {
        ...
        void (*TxCpltCallback)(
            struct __SMARTCARD_HandleTypeDef *hsc); /*!< SMARTCARD Tx Complete Callback */

        void (*RxCpltCallback)(
            struct __SMARTCARD_HandleTypeDef *hsc); /*!< SMARTCARD Rx Complete Callback */

        void (*ErrorCallback)(
            struct __SMARTCARD_HandleTypeDef *hsc); /*!< SMARTCARD Error Callback */
        ...
        void (*AbortTransmitCpltCallback)(
            struct __SMARTCARD_HandleTypeDef
                *hsc); /*!< SMARTCARD Abort Transmit Complete Callback */

        void (*AbortReceiveCpltCallback)(struct __SMARTCARD_HandleTypeDef *
                                             hsc); /*!< SMARTCARD Abort Receive Complete Callback */

        void (*MspInitCallback)(
            struct __SMARTCARD_HandleTypeDef *hsc); /*!< SMARTCARD Msp Init callback */
 ...
 *         SMARTCARD Handle selects the USARTx peripheral (USART availability and x value depending
 * on device).
 * @retval None
 */
#if USE_HAL_SMARTCARD_REGISTER_CALLBACKS == 1
...
#define __HAL_SMARTCARD_DMA_REQUEST_DISABLE(__HANDLE__, __REQUEST__)                               \
    ...
    HAL_StatusTypeDef HAL_SMARTCARD_Receive(SMARTCARD_HandleTypeDef *hsc,
                                            ...
                                            uint32_t                 Timeout);
    HAL_StatusTypeDef
    HAL_SMARTCARD_Transmit_IT(SMARTCARD_HandleTypeDef *hsc, const uint8_t *pData, uint16_t Size);
    HAL_StatusTypeDef
    HAL_SMARTCARD_Receive_IT(SMARTCARD_HandleTypeDef *hsc, uint8_t *pData, uint16_t Size);
    HAL_StatusTypeDef
    HAL_SMARTCARD_Transmit_DMA(SMARTCARD_HandleTypeDef *hsc, const uint8_t *pData, uint16_t Size);
    HAL_StatusTypeDef
    HAL_SMARTCARD_Receive_DMA(SMARTCARD_HandleTypeDef *hsc, uint8_t *pData, uint16_t Size);
