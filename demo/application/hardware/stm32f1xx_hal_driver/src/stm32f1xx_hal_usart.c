  ...
  the DMA Tx/Rx channel.
    ...
    (#) Initialize the USART registers by calling the HAL_USART_Init() API:
        ...
        -@@- The specific USART interrupts (Transmission complete interrupt,
             ...
             __HAL_USART_ENABLE_IT() and __HAL_USART_DISABLE_IT() inside the transmit and receive
  process.
     ...
     *** DMA mode IO operation ***
     ...
     [..]
       ...
       (+) At transmission end of half transfer HAL_USART_TxHalfCpltCallback is executed and user
  can add his own code by customization of function pointer HAL_USART_TxHalfCpltCallback
                                            ...
                                            uint32_t             Timeout)
{
    ...
    if(husart->State == HAL_USART_STATE_READY)
    {
        ...
        /* In case of 9bits/No Parity transfer, pTxData and pRxData buffers provided as input
           parameter should be aligned on a u16 frontier, as data to be filled into TDR/retrieved
 ...
 */
HAL_StatusTypeDef
HAL_USART_Transmit_IT(USART_HandleTypeDef *husart, const uint8_t *pTxData, uint16_t Size)
{
    if(husart->State == HAL_USART_STATE_READY)
    {
        if((pTxData == NULL) || (Size == 0))
 ...
 */
HAL_StatusTypeDef
HAL_USART_Transmit_DMA(USART_HandleTypeDef *husart, const uint8_t *pTxData, uint16_t Size)
{
    HAL_StatusTypeDef status = HAL_OK;
    const uint32_t   *tmp;
    ...
    if(husart->State == HAL_USART_STATE_READY)
    {
        ...
        status = HAL_DMA_Start_IT(husart->hdmatx,
                                  *(const uint32_t *)tmp,
                                  (uint32_t)&husart->Instance->DR,
                                  Size);
        if(status == HAL_OK)
        {
...
HAL_USART_Receive_DMA(USART_HandleTypeDef *husart, uint8_t *pRxData, uint16_t Size)
{
    ...
    if(husart->State == HAL_USART_STATE_READY)
    {
        ...
        status = HAL_DMA_Start_IT(husart->hdmarx,
                                  (uint32_t)&husart->Instance->DR,
                                  *(uint32_t *)tmp,
                                  Size);
        ...
        if(status == HAL_OK)
        {
            /* Clear the Overrun flag just before enabling the DMA Rx request: mandatory for the
             * second transfer */
...
HAL_StatusTypeDef HAL_USART_DMAStop(USART_HandleTypeDef *husart)
{
    ...
    /* The Lock is not implemented on this API to allow the user application
       ...
       HAL_USART_RxCpltCallback(): when calling HAL_DMA_Abort() API the DMA TX/RX Transfer complete
       interrupt is generated and the correspond call back is executed HAL_USART_TxCpltCallback() /
       HAL_USART_RxCpltCallback()
       */
...
/**
 * @brief  End ongoing Tx transfer on USART peripheral (following error detection or Transmit
 * completion).
 * @param  husart USART handle.
 * @retval None
 */
...
/**
 * @brief  End ongoing Rx transfer on USART peripheral (following error detection or Reception
 * completion).
 * @param  husart USART handle.
 * @retval None
 */
...
static void USART_SetConfig(USART_HandleTypeDef *husart)
{
    ...
    tmpreg &= (uint32_t) ~((uint32_t)(USART_CR2_CPHA | USART_CR2_CPOL | USART_CR2_CLKEN |
