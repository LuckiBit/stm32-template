...
#include "stm32f1xx_hal_def.h"
    ...
    typedef struct
    {
        ...
        uint32_t WordLength; /*!< Specifies the number of data bits transmitted or received in a
                                frame. This parameter can be a value of @ref USART_Word_Length */
 ...
 *         USART Handle selects the USARTx peripheral (USART availability and x value depending on
 * device).
 * @retval None
 */
#if(USE_HAL_USART_REGISTER_CALLBACKS == 1)
...
#define USART_DIVFRAQ(_PCLK_, _BAUD_)                                                              \
    ((((USART_DIV((_PCLK_), (_BAUD_)) - (USART_DIVMANT((_PCLK_), (_BAUD_)) * 100U)) * 16U) +       \
      50U) /                                                                                       \
     100U)
