...
| 项目 | 当前实现 |
| --- | --- |
| 目标芯片 | STM32F103RCT6，Cortex-M3，256 KiB Flash、48 KiB SRAM |
...
| `hardware` | CMSIS、HAL、板级资源和具体器件驱动 | GPIO、PC10 LED、Application 串口、芯片支持代码 |
...
这些时间表示翻转间隔，不是完整亮灭周期。BSP 当前使用低有效 PC10 LED。
...
| Boot Info | `0x0803F800` | `0x08040000` | 2 KiB，独占一个擦除页 |

SRAM 范围为 `0x20000000`～`0x2000C000`，容量 48 KiB。
...
FreeRTOS 移植层以及两套链接脚本和分区常量。当前只配置了 STM32F103RCT6，
