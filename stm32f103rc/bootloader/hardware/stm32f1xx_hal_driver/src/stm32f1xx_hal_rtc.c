  ...
  *** Alarm configuration ***
  ...
  [..]
    (+) To configure the RTC Alarm use the HAL_RTC_SetAlarm() function.
        You can also configure the RTC Alarm with interrupt mode using the HAL_RTC_SetAlarm_IT()
  function.
...
static void RTC_DateUpdate(RTC_HandleTypeDef *hrtc, uint32_t DayElapsed)
{
    ...
    {
        if((month == 1U) || (month == 3U) || (month == 5U) || (month == 7U) || (month == 8U) ||
           (month == 10U) || (month == 12U))
        {
            if(day < 31U)
            {
...
static uint8_t RTC_WeekDayNum(uint32_t nYear, uint8_t nMonth, uint8_t nDay)
{
    ...
    if(nMonth < 3U)
    {
        /*D = { [(23 x month)/9] + day + 4 + year + [(year-1)/4] - [(year-1)/100] + [(year-1)/400] }
         * mod 7*/
        weekday = (((23U * nMonth) / 9U) + nDay + 4U + year + ((year - 1U) / 4U) -
                   ((year - 1U) / 100U) + ((year - 1U) / 400U)) %
                  7U;
    }
    else
    {
        /*D = { [(23 x month)/9] + day + 4 + year + [year/4] - [year/100] + [year/400] - 2 } mod 7*/
        weekday = (((23U * nMonth) / 9U) + nDay + 4U + year + (year / 4U) - (year / 100U) +
                   (year / 400U) - 2U) %
                  7U;
    }
