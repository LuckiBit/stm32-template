                                         ...
                                         uint32_t           NumberOfBlocks)
{
    ...
    if(hmmc->State == HAL_MMC_STATE_READY)
    {
        ...
        if(HAL_DMA_Start_IT(hmmc->hdmarx,
                            (uint32_t)&hmmc->Instance->FIFO,
                            (uint32_t)pData,
                                          ...
                                          uint32_t           NumberOfBlocks)
{
    ...
    if(hmmc->State == HAL_MMC_STATE_READY)
    {
        ...
        if(errorstate != HAL_MMC_ERROR_NONE)
        {
            ...
            __HAL_MMC_DISABLE_IT(
                hmmc,
                (SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_TXUNDERR | SDIO_IT_DATAEND));
            hmmc->ErrorCode |= errorstate;
            hmmc->State = HAL_MMC_STATE_READY;
            return HAL_ERROR;
        }
        ...
        /* Enable the DMA Channel */
        if(HAL_DMA_Start_IT(hmmc->hdmatx,
                            (uint32_t)pData,
                            (uint32_t)&hmmc->Instance->FIFO,
                            (uint32_t)(MMC_BLOCKSIZE * NumberOfBlocks) / 4) != HAL_OK)
        {
...
void HAL_MMC_IRQHandler(MMC_HandleTypeDef *hmmc)
{
    ...
    else if(__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_DATAEND) != RESET)
    {
        ...
        __HAL_MMC_DISABLE_IT(hmmc,
                             SDIO_IT_DATAEND | SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT |
            ...
            ((context & MMC_CONTEXT_IT) != 0U))
    {
        MMC_Write_IT(hmmc);
                               ...
                               SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_RXOVERR |
                                   SDIO_FLAG_TXUNDERR) != RESET)
    {
        ...
        else if((context & MMC_CONTEXT_DMA) != 0U)
        {
            ...
            else if(((context & MMC_CONTEXT_READ_SINGLE_BLOCK) != 0U) ||
                    ((context & MMC_CONTEXT_READ_MULTIPLE_BLOCK) != 0U))
            {
                if(hmmc->hdmarx != NULL)
                {
...
HAL_StatusTypeDef HAL_MMC_Abort(MMC_HandleTypeDef *hmmc)
{
    ...
    /* DIsable All interrupts */
    __HAL_MMC_DISABLE_IT(hmmc,
                         SDIO_IT_DATAEND | SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_TXUNDERR |
                             SDIO_IT_RXOVERR);
 ...
 */
static uint32_t
MMC_ReadExtCSD(MMC_HandleTypeDef *hmmc, uint32_t *pFieldData, uint16_t FieldIndex, uint32_t Timeout)
{
    SDIO_DataInitTypeDef config;
    uint32_t             errorstate;
    uint32_t             tickstart = HAL_GetTick();
