...
| 项目 | 当前实现 |
| --- | --- |
| 目标芯片 | STM32F429IGT6，Cortex-M4F，1024 KiB Flash、192 KiB SRAM、64 KiB CCMRAM |
| 编译工具链 | GCC `arm-none-eabi-gcc`，C11，CMake、Ninja |
| 编译告警 | 两个工程均启用 `-Wall -Wextra` |
| 外设驱动 | 两个工程均编译 STM32F4 HAL，芯片宏为 `STM32F429xx` |
| Application RTOS | FreeRTOS，GCC `arm_cm4f` 移植层，单核抢占调度，硬件浮点 ABI |
...
`TASK_ENABLE_n` 等开关集中在 `app/tasks/tasks.h`，默认仅 `TASK_ENABLE_4=1`。
...
| Application | `0x08008000` | `0x080E0000` | 864 KiB |
| Boot Info | `0x080E0000` | `0x08100000` | 128 KiB，独占 Flash 扇区 11 |

主 SRAM 范围为 `0x20000000`～`0x20030000`，容量 192 KiB；CCMRAM 范围为
`0x10000000`～`0x10010000`，容量 64 KiB。当前普通数据、堆和栈使用主 SRAM，
仅显式放入 `.ccmram` 段的数据使用 CCMRAM。

配置位置：

- [Bootloader 分区常量](../bootloader/core/flash_layout.h)。
- [Application 分区常量](../application/core/flash_layout.h)。
- [Bootloader 链接布局](../bootloader/core/linker/stm32f429ig.ld)。
- [Application 链接布局](../application/core/linker/stm32f429ig.ld)。

两个工程的 `core/linker/` 均只保存 F429IG 对应的链接脚本，CMake 直接引用该脚本。

两份 `flash_layout.h` 分别维护，修改分区时必须同步，链接脚本的数值也必须一致。
Bootloader 检查和跳转使用 `APP_START_ADDRESS`；Application 的
[`SystemInit`](../application/core/system_clock/system_stm32f4xx.c) 也使用该宏设置向量表。
...
更改分区后，还需检查扇区对齐、区域边界、两份常量与链接布局、向量入口和跳转行为。
...
FreeRTOS 移植层以及两套链接脚本和分区常量。当前只配置了 STM32F429IGT6，
