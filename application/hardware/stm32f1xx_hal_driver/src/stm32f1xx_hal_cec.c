...
#define CEC_ESR_ALL_ERROR                                                                          \
    (CEC_ESR_BTE | CEC_ESR_BPE | CEC_ESR_RBTFE | CEC_ESR_SBE | CEC_ESR_ACKE | CEC_ESR_LINE |       \
     CEC_ESR_TBTFE)
#define CEC_RXXFERSIZE_INITIALIZE                                                                  \
    0xFFFF /*!< Value used to initialise the RxXferSize of the handle */
/**
 * @}
 */
...
HAL_StatusTypeDef HAL_CEC_Init(CEC_HandleTypeDef *hcec)
{
    ...
    MODIFY_REG(hcec->Instance->CFGR,
               CEC_CFGR_FIELDS,
               hcec->Init.TimingErrorFree | hcec->Init.PeriodErrorFree);
                                      ...
                                      uint32_t           Size)
{
    ...
    if(hcec->gState == HAL_CEC_STATE_READY)
    {
        ...
        /* send header block */
        hcec->Instance->TXD =
            (uint8_t)((uint32_t)InitiatorAddress << CEC_INITIATOR_LSB_POS) | DestinationAddress;
...
static HAL_StatusTypeDef CEC_Transmit_IT(CEC_HandleTypeDef *hcec)
{
    ...
    if((hcec->gState == HAL_CEC_STATE_BUSY_TX) ||
