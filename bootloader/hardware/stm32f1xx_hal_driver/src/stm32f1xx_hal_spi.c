 ...
 *
 * Check RevisionID value for identifying if Device is Rev Z (0x0001) in order to enable workaround
 * for CRC errors wrongly detected
 */
/* Pb is that ES_STM32F10xxCDE also identify an issue in Debug registers access while not in Debug
 * mode Revision ID information is only available in Debug mode, so Workaround could not be
 * implemented to distinguish Rev Z devices (issue present) from more recent version (issue fixed).
...
HAL_SPI_Receive(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
    ...
    if(hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE)
    {
        ...
        {
            /* Workaround to force CRCNEXT bit to zero in case of CRCNEXT is not reset automatically
             * by hardware */
...
static void SPI_RxISR_8BIT(struct __SPI_HandleTypeDef *hspi)
{
    *hspi->pRxBuffPtr = (*(__IO uint8_t *)&hspi->Instance->DR);
    ...
    if(READ_BIT(hspi->Instance->CR1, SPI_CR1_CRCNEXT))
    {
        /* Workaround to force CRCNEXT bit to zero in case of CRCNEXT is not reset automatically by
         * hardware */
        CLEAR_BIT(hspi->Instance->CR1, SPI_CR1_CRCNEXT);
    }
...
uint8_t SPI_ISCRCErrorValid(SPI_HandleTypeDef *hspi)
{
    ...
    /* If CRC errors could be wrongly detected (issue 2.15.2 in STM32F10xxC/D/E silicon limitations
       ES (DocID14732 Rev 13)) */
