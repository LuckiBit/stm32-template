...
#include "stm32f1xx_hal_def.h"
     ...
     * parameter 'ExternalTrigInjecConv')
     *          - For all except parameters 'ExternalTrigInjecConv': ADC enabled without conversion
    ...
    typedef struct
    {
        uint32_t
            InjectedChannel;   /*!< Selection of ADC channel to configure
                                  ...
                                  package pins. Refer to device datasheet for channels availability.
                                    Note: On STM32F1 devices with several ADC: Only ADC1 can access
                                  internal measurement channels (VrefInt/TempSensor)   Note: On
                                  STM32F10xx8 and STM32F10xxB devices: A low-amplitude voltage glitch
                                  may be generated (on ADC input 0) on the PA0 pin, when the ADC is
                                  converting with injection trigger.   It is advised to distribute the
                                  analog channels so that Channel 0 is configured as an injected
        ...
        uint32_t
            InjectedSamplingTime; /*!< Sampling time value to be set for the selected channel.
                                     ...
                                     bits). This parameter can be a value of @ref ADC_sampling_times
                                     ...
                                     channels (VrefInt/TempSensor), sampling time constraints must
                                     be respected (sampling time can be adjusted in function of ADC
        ...
        uint32_t
            ExternalTrigInjecConv; /*!< Selects the external event used to trigger the conversion
                                      ...
                                      ADC_INJECTED_SOFTWARE_START, external triggers are disabled.
                                        If set to external trigger source, triggering is on event
                                      rising edge. This parameter can be a value of @ref
                                      ADCEx_External_trigger_source_Injected Note: This parameter
                                      must be modified when ADC is disabled (before ADC start
                                      conversion or after ADC stop conversion). If ADC is enabled,
                                      this parameter setting is bypassed without error reporting (as
                                      it can be the expected behaviour in case of another parameter
                                      update on the fly) Caution: this setting impacts the entire
                                      injected group. Therefore, call of
                                      HAL_ADCEx_InjectedConfigChannel() to configure a channel on
                                      injected group can impact the configuration of other channels
                                      previously set. */
...
#if defined(STM32F103x6) || defined(STM32F103xB) || defined(STM32F105xC) ||                        \
    defined(STM32F107xC) || defined(STM32F103xE) || defined(STM32F103xG)
    /**
     * @brief  Structure definition of ADC multimode
     * @note   The setting of these parameters with function HAL_ADCEx_MultiModeConfigChannel() is
     * conditioned to ADCs state (both ADCs of the common group). State of ADCs of the common group
     * must be: disabled.
     */
    typedef struct
    {
        uint32_t
            Mode; /*!< Configures the ADC to operate in independent or multi mode.
                     ...
                     mode before any configuration change. Note: In case of simultaneous mode used:
                     Exactly the same sampling time should be configured for the 2 channels that
                     will be sampled simultaneously by ACD1 and ADC2. Note: In case of interleaved
                     mode used: To avoid overlap between conversions, maximum sampling time allowed
                     is 7 ADC clock cycles for fast interleaved mode and 14 ADC clock cycles for
                     slow interleaved mode. Note: Some multimode parameters are fixed on STM32F1 and
                     can be configured on other STM32 devices with several ADC (multimode
                     configuration structure can have additional parameters). The equivalences are:
                               - Parameter 'DMAAccessMode': On STM32F1, this parameter is fixed to 1
                     DMA channel (one DMA channel for both ADC, DMA of ADC master). On other STM32
                     devices with several ADC, this is equivalent to parameter
                     'ADC_DMAACCESSMODE_12_10_BITS'.
                               - Parameter 'TwoSamplingDelay': On STM32F1, this parameter is fixed
                     to 7 or 14 ADC clock cycles depending on fast or slow interleaved mode
                     selected. On other STM32 devices with several ADC, this is equivalent to
                     parameter 'ADC_TWOSAMPLINGDELAY_7CYCLES' (for fast interleaved mode). */

    } ADC_MultiModeTypeDef;
#endif /* defined STM32F103x6 || defined STM32F103xB || defined STM32F105xC || defined STM32F107xC \
...
/** @defgroup ADCEx_External_trigger_source_Injected ADCEx External trigger selection for injected
 * group
 * @{
 */
/*!< List of external triggers with generic trigger name, independently of    */
...
#define ADC_DUALMODE_REGSIMULT_INJECSIMULT                                                         \
    ((uint32_t)(ADC_CR1_DUALMOD_0)) /*!< ADC dual mode enabled: Combined regular simultaneous +    \
                                       injected simultaneous mode, on groups regular and injected  \
                                     */
#define ADC_DUALMODE_REGSIMULT_ALTERTRIG                                                           \
    ((uint32_t)(ADC_CR1_DUALMOD_1)) /*!< ADC dual mode enabled: Combined regular simultaneous +    \
                                       alternate trigger mode, on groups regular and injected */
#define ADC_DUALMODE_INJECSIMULT_INTERLFAST                                                        \
    ((uint32_t)(ADC_CR1_DUALMOD_1 |                                                                \
                ADC_CR1_DUALMOD_0)) /*!< ADC dual mode enabled: Combined injected simultaneous +   \
                                       fast interleaved mode, on groups regular and injected       \
                                       (delay between ADC sampling phases: 7 ADC clock cycles      \
                                       (equivalent to parameter "TwoSamplingDelay" set to          \
                                       "ADC_TWOSAMPLINGDELAY_7CYCLES" on other STM32 devices)) */
#define ADC_DUALMODE_INJECSIMULT_INTERLSLOW                                                        \
    ((uint32_t)(ADC_CR1_DUALMOD_2)) /*!< ADC dual mode enabled: Combined injected simultaneous +   \
                                       slow Interleaved mode, on groups regular and injected       \
                                       (delay between ADC sampling phases: 14 ADC clock cycles     \
                                       (equivalent to parameter "TwoSamplingDelay" set to          \
                                       "ADC_TWOSAMPLINGDELAY_7CYCLES" on other STM32 devices)) */
#define ADC_DUALMODE_INJECSIMULT                                                                   \
    ((uint32_t)(ADC_CR1_DUALMOD_2 | ADC_CR1_DUALMOD_0)) /*!< ADC dual mode enabled: Injected       \
                                                           simultaneous mode, on group injected */
#define ADC_DUALMODE_REGSIMULT                                                                     \
    ((uint32_t)(ADC_CR1_DUALMOD_2 | ADC_CR1_DUALMOD_1)) /*!< ADC dual mode enabled: Regular        \
                                                           simultaneous mode, on group regular */
#define ADC_DUALMODE_INTERLFAST                                                                    \
    ((uint32_t)(ADC_CR1_DUALMOD_2 | ADC_CR1_DUALMOD_1 |                                            \
                ADC_CR1_DUALMOD_0)) /*!< ADC dual mode enabled: Fast interleaved mode, on group    \
                                       regular (delay between ADC sampling phases: 7 ADC clock     \
                                       cycles (equivalent to parameter "TwoSamplingDelay" set to   \
                                       "ADC_TWOSAMPLINGDELAY_7CYCLES" on other STM32 devices)) */
#define ADC_DUALMODE_INTERLSLOW                                                                    \
    ((uint32_t)(ADC_CR1_DUALMOD_3)) /*!< ADC dual mode enabled: Slow interleaved mode, on group    \
                                       regular (delay between ADC sampling phases: 14 ADC clock    \
                                       cycles (equivalent to parameter "TwoSamplingDelay" set to   \
                                       "ADC_TWOSAMPLINGDELAY_7CYCLES" on other STM32 devices)) */
#define ADC_DUALMODE_ALTERTRIG                                                                     \
    ((uint32_t)(ADC_CR1_DUALMOD_3 | ADC_CR1_DUALMOD_0)) /*!< ADC dual mode enabled: Alternate      \
                                                           trigger mode, on group injected */
/**
 * @}
 */
#endif /* defined STM32F103x6 || defined STM32F103xB || defined STM32F105xC || defined STM32F107xC \
...
/** @defgroup ADCEx_Internal_HAL_driver_Ext_trig_src_Regular ADC Extended Internal HAL driver
...
/** @defgroup ADCEx_Internal_HAL_driver_Ext_trig_src_Injected ADC Extended Internal HAL driver
 * trigger selection for injected group
 * @{
 */
...
#define ADC_CFGR_EXTSEL(__HANDLE__, __EXT_TRIG_CONV__)                                             \
    (((((__HANDLE__)->Instance) == ADC3))                                                          \
         ? (((__EXT_TRIG_CONV__) == ADC_EXTERNALTRIGCONV_T8_TRGO) ? (ADC3_EXTERNALTRIG_T8_TRGO)    \
                                                                  : (__EXT_TRIG_CONV__))           \
         : (__EXT_TRIG_CONV__))
#else
...
/**
 * @brief Verification if multimode is enabled for the selected ADC (multimode ADC master or ADC
 * slave) (applicable for devices with several ADCs)
 * @param __HANDLE__: ADC handle
 ...
 * multimode with handle of ADC master (applicable for devices with several ADCs)
 * @param __HANDLE__: ADC handle
 * @retval None
 */
#if defined(STM32F103x6) || defined(STM32F103xB) || defined(STM32F105xC) ||                        \
...
/**
 * @brief Check ADC multimode setting: In case of multimode, check whether ADC master of the
 * selected ADC has feature auto-injection enabled (applicable for devices with several ADCs)
 * @param __HANDLE__: ADC handle
 * @retval None
 */
#if defined(STM32F103x6) || defined(STM32F103xB) || defined(STM32F105xC) ||                        \
...
#define IS_ADC_INJECTED_RANK(CHANNEL)                                                              \
    (((CHANNEL) == ADC_INJECTED_RANK_1) || ((CHANNEL) == ADC_INJECTED_RANK_2) ||                   \
...
#define IS_ADC_INJECTED_NB_CONV(LENGTH) (((LENGTH) >= 1U) && ((LENGTH) <= 4U))
    ...
    defined(STM32F101xB) || defined(STM32F102x6) || defined(STM32F102xB) ||                        \
